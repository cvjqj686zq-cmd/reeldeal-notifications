...
// ── Claude FMV analysis with retry ────────────────────────────────────────
async function analyzeWithClaude(listings, threshold, minProfit, studioList, attempt = 1) {
...
{"itemId":"","title":"","studio":"","isBoutique":true,"askingPrice":0,"estimatedFMV":0,"discountPct":0,"estimatedProfit":0,"isDeal":false,"isHot":false,"isOOP":false,"reasoning":"","resaleNotes":"","url":""}`;
  ...
  console.log('Claude HTTP status:', result.status);

  // Retry on overload (529) or server error (500) — up to 3 attempts
  if (result.body?.error?.type === 'overloaded_error' || result.status === 529 || result.status === 500) {
    if (attempt <= 3) {
      const wait = attempt * 5000; // 5s, 10s, 15s
      console.log(`Claude overloaded — retrying in ${wait/1000}s (attempt ${attempt}/3)...`);
      await new Promise(r => setTimeout(r, wait));
      return analyzeWithClaude(listings, threshold, minProfit, studioList, attempt + 1);
    }
    throw new Error('Claude API overloaded after 3 attempts. Try again in a minute.');
  }

  if (!result.body) throw new Error('Claude returned empty body');
  if (result.body.error) throw new Error('Claude API error: ' + result.body.error.message);
  if (!result.body.content) {
    console.error('Claude full response:', JSON.stringify(result.body).substring(0, 500));
    throw new Error('Claude response missing content. Stop reason: ' + result.body.stop_reason);
  }
  ...
  if (!match) throw new Error('Claude response did not contain a JSON array: ' + cleaned.substring(0, 300));
...
app.post('/scan', async (req, res) => {
  ...
  // Map condition strings to eBay numeric IDs
  // 1000=New, 1500=New other, 2500=Like New, 3000=Very Good, 4000=Good, 5000=Acceptable
  const condIdLookup = {
    'NEW': '1000|1500', 'LIKE_NEW': '2500', 'VERY_GOOD': '3000',
    'GOOD': '4000', 'ACCEPTABLE': '5000',
  };
  const condIds = condition
    ? condition.split('|').flatMap(c => (condIdLookup[c] || '').split('|')).filter(Boolean).join('|')
    : '';
  // eBay Browse API conditionIds filter — pipe-separated IDs inside curly braces
  const condFilter = condIds ? `&filter=conditionIds:{${condIds}}` : '';

  // Format: may be pipe-separated list from checkboxes — search each separately
  const formatList = format ? format.split('|').filter(Boolean) : [''];

  try {
    const token = await getToken(clientId, clientSecret);
    const studioList = (studios && studios.length > 0) ? studios : ['Criterion', 'Arrow Video', 'Vinegar Syndrome'];

    let allListings = [];
    for (const studio of studioList) {
      for (const fmt of formatList) {
        const q = fmt ? `${studio} ${fmt}` : studio;
        const baseFilter = `buyingOptions:{FIXED_PRICE},price:[0..${maxPrice}],priceCurrency:USD`;
        const fullFilter = condIds ? `${baseFilter},conditionIds:{${condIds}}` : baseFilter;
        const path = `/buy/browse/v1/item_summary/search?q=${encodeURIComponent(q)}&filter=${encodeURIComponent(fullFilter)}&category_ids=617&limit=${maxResults}&sort=price`;
        try {
          ...
          console.log(`  ${studio}${fmt?' '+fmt:''}: ${items.length} listings`);
    ...
    // Fetch sold comps in batches of 5 to avoid overwhelming the Finding API
    const compsResults = [];
    for (let i = 0; i < allListings.length; i += 5) {
      ...
      compsResults.push(...batchResults);
      if (i + 5 < allListings.length) await new Promise(r => setTimeout(r, 300));
    }
    ...
    const raw = await analyzeWithClaude(payload, threshold, minProfit, studioList);
    if (!Array.isArray(raw)) throw new Error('Claude did not return an array. Got: ' + JSON.stringify(raw).substring(0, 200));
    const analyzed = raw.filter(item => item.isBoutique !== false);
