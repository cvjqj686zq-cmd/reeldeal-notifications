...
{"itemId":"","title":"","studio":"","isBoutique":true,"askingPrice":0,"estimatedFMV":0,"discountPct":0,"estimatedProfit":0,"isDeal":false,"isHot":false,"isOOP":false,"reasoning":"","resaleNotes":"","url":""}`;
  ...
  if (!result.body.content) {
    console.error('Claude response missing content:', JSON.stringify(result.body));
    throw new Error('Claude returned no content: ' + JSON.stringify(result.body));
  }
  const text = result.body.content.map(c => c.text || '').join('');
  const cleaned = text.replace(/```json|```/g, '').trim();
  // Extract JSON array even if Claude adds preamble text
  const match = cleaned.match(/\[[\s\S]*\]/);
  if (!match) throw new Error('Claude response did not contain a JSON array: ' + cleaned.substring(0, 200));
